...
from utils.crawler import CrawlerError, YouTubeCrawler
from utils.text_cleaner import clean_comment, is_valid_url, is_youtube_url
...
def _get_cached_analyzer(model_name: str, model_candidates: tuple[str, ...]) -> SentimentAnalyzer:
    return SentimentAnalyzer(model_name=model_name, model_candidates=list(model_candidates))


def build_analyzer() -> SentimentAnalyzer:
    return _get_cached_analyzer(
        current_app.config["SENTIMENT_MODEL_NAME"],
        tuple(current_app.config["SENTIMENT_MODEL_CANDIDATES"]),
    )
...
def analyze_and_store_comments(source_url: str, source_type: str, comments: List[str]) -> int:
...
def analyze():
    ...
    if not is_valid_url(source_url) or not is_youtube_url(source_url):
        flash("URL khong hop le", "error")
        return redirect(url_for("main.index"))

    crawler = YouTubeCrawler(max_comments=current_app.config["MAX_COMMENTS"])

    try:
        raw_comments = crawler.extract_comments(source_url)
        analysis_run_id = analyze_and_store_comments(source_url, "youtube", raw_comments)
...
def analyze_text():
    ...
    try:
        analysis_run_id = analyze_and_store_comments("Nhap thu cong", "manual", comments)
...
def result():
    analysis_run_id = request.args.get("analysis_run_id", type=int) or session.get("last_analysis_run_id")
